...
## `robust_linf.py`
Uncertainty set: $ \Theta = \{\theta : \|\theta - c\|_\infty \le r\} $.
...
## `robust_ellipsoid.py`
Uncertainty set: Ellipsoid  
$ \Theta = \{\theta : (\theta - c)^\top W (\theta - c) \le r^2\} $, $ W \succeq 0 $.
...
Robust constraint:  
$ \langle w, c \rangle + r \sqrt{w^\top W^{-1} w} \le 1 $.
